...
REM Starts Redis (Docker), the API + dispatch worker, and the React dev server
...
start "jobfair-worker" cmd /k "cd /d "%API_DIR%" && npm run worker"
...
echo   - jobfair-worker (BullMQ slot dispatcher)
